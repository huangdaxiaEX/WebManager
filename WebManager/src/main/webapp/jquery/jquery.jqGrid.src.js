...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		loadData=function(propertys, condition, orderby, sord, page, rows, dt, rcnt, npage, adjust, lc, pvis){
			if(ts.p.dwrFun == undefined){
				alert("缺少获取数据的方法：dwrFun");
				return;
			}
			if(ts.p.ptypes){
				ts.p.dwrFun(propertys,ts.p.ptypes,condition,orderby,sord,ts.p.needLink,page,rows,
					function(data){
						var gridData={
							page:ts.p.page,
							total:ts.p.total,
						...
						if(ts.p.page>ts.p.total&&ts.p.total>-1){
							gridData.page=ts.p.total;
						}
						if(gridData){
							addJSONData(gridData,ts.grid.bDiv,rcnt,npage>1,adjust);
							if(lc) { lc.call(ts,gridData); }
				...
				ts.p.dwrFun(propertys,condition,orderby,sord,ts.p.needLink,page,rows,
					function(data){
						var gridData={
							page:ts.p.page,
							total:ts.p.total,
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				case "json":
					if(ts.p.dwr){
						var obj = $.isFunction(ts.p.serializeGridData)? ts.p.serializeGridData.call(ts,ts.p.postData) : ts.p.postData;
						var page = obj.page;
						var rows = obj.rows;
						var orderby = obj.sidx;
						var sord = obj.sord;
						
						
						//处理查询条件
						var dc = ts.p.condition;
						var sc = ts.p.searchCondition;
						var selc = ts.p.selectCondition;
						var condition = '';
						if(dc == undefined && sc != undefined){
							condition = sc;
						}else if(dc == undefined && sc != undefined){
							condition = dc;
						}else if(dc != undefined&&sc!= undefined){
							if(dc==''&&sc!=''){
								condition = sc;
						...
						if(selc != undefined && selc.length > 0){
						...
						var proLenght = ts.p.colModel.length;
						var propertys = [];
						var index,i;
						for (index=0,i=0;index<proLenght;index++){
							if(ts.p.colModel[index].index != null){
								propertys[i++] = (ts.p.colModel[index]).index;
							}
						};
						if(ts.p.total==-1 || ts.p.total==undefined){
							if(ts.p.dwrCountFun == undefined){
								alert("缺少获取数据总数的方法：dwrCountFun");
								break;
							}
							ts.p.dwrCountFun(propertys,condition,ts.p.needLink,function(data){
								if(data != undefined && data != null){//返回总条数  2012.9.6修改
									ts.p.records = data;
									var totalPage = 0;
									if(data == 0){
										totalPage = 0;
										...
										if(data%10==0){
											totalPage = parseInt(data/rows);
										}
										else{
											totalPage = parseInt(data/rows) + 1;
										}
									}
									ts.p.total = totalPage;
									if(page>totalPage&&totalPage>-1){
										page=totalPage;
									}
									loadData(propertys, condition, orderby, sord, page, rows, dt, rcnt, npage, adjust, lc, pvis);
								}
							});
						}else{
							//处理调整每页显示后的总页数  2012.9.6修改
							var totalPage = 0;
							if(ts.p.records == 0){
								totalPage = 0;
							}else if(ts.p.records <= rows){
								totalPage = 1;
							}else{
								if(ts.p.records%10==0){
									totalPage = parseInt(ts.p.records/rows);
								}
								else{
									totalPage = parseInt(ts.p.records/rows) + 1;
								}
							}
							ts.p.total = totalPage;
							loadData(propertys, condition, orderby, sord, page, rows, dt, rcnt, npage, adjust, lc, pvis);
        ...
        setPager = function (pgid, tp){
			...
			if(dir=="ltr") { pgl += str; }
			pgl += "</tr></tbody></table>";
			if(ts.p.viewrecords===true) {$("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<div dir='"+dir+"' style='text-align:"+ts.p.recordpos+"; padding-right:15px;' class='ui-paging-info'></div>");}
		...
		$(grid.bDiv)
			...
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width)+"px", overflow: "hidden"})
