...
(function($) {
	...
	$.fn.zTree = function(zTreeSetting, zTreeNodes) {
		...
		var setting = {
			...
			dwrFun : null,
			needLink:false,
			isLeaf:function(node){return false;},//判断节点是否是叶节点
			loadConditionFun:null,//在加载下级节点的时候加入的判断条件
			properties : ['*'],
	 		condition:'',
	 		sortField:'',
	 		order:'',
	 		nameFormatter:null,
	 		isMixData:false,//是否是混合数据,例如树中有单位又有人员的情况
	 		mixData:{},//混合数据的对象json格式
	 		mixDataSelectHandler:function(node){return null},
	 		curDataName:'',//当前获取的混合数据的名称,mixData中的名称,这个是组件用的
	 		//二次开发加入到此为止-------------------------------------------
			//event Function
			callback: {
				beforeAsync:null,
	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			...
			node.isAjaxing = null;
			addCache(setting, node);//这里就将用户的节点数据存入
			...
			html.push("<a id='", node.tId, IDMark_A, "' treeNode", IDMark_A," onclick=\"", (node.click || ''),
				...
				"'>",/*node[node.nameCol ? node.nameCol : setting.nameCol]*/shownameFormatter(node[node.nameCol ? node.nameCol : setting.nameCol],node,setting).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span></a><ul id='", node.tId, IDMark_Ul, "' class='", makeUlLineClass(setting, node), "' style='display:", (node.open ? "block": "none"),"'>");
	...
	function shownameFormatter(name,node,setting){
		if(!name){
			alert("节点nameCol错误setting.nameCol="+setting.nameCol);
		}
		if(setting.nameFormatter != null){
			result = setting.nameFormatter(name,node);
	...
	var st = {
		...
		cancelPreEditNode: function (setting, newName) {
			if (setting.curEditTreeNode) {
				...
				setting.curEditTreeNode[setting.curEditTreeNode.nameCol ? setting.curEditTreeNode.nameCol : setting.nameCol] = newName ? newName:inputObj.val();
				//触发rename事件
...
//		nObj.text(treeNode[treeNode.nameCol ? treeNode.nameCol : setting.nameCol]);
//		nObj.text(shownameFormatter(treeNode[treeNode.nameCol ? treeNode.nameCol : setting.nameCol],treeNode,setting));
		var node = $(treeNode)[0];
		var nObj = $("#" + node.tId + IDMark_Span);
		nObj.text(shownameFormatter(node[node.nameCol ? node.nameCol : setting.nameCol],treeNode,setting));
	}
	//设置Target
	...
	function asyncGetNode(setting, treeNode) {
		...
		//这里是二次开发加入的-------------------------------------------
		var condition = treeNode.condition!=undefined ? treeNode.condition : setting.condition;
		//处理用户自定义的查询条件
		if(setting.loadConditionFun){
			var userCondition = setting.loadConditionFun(treeNode);
			if(userCondition && userCondition != ''){
				condition = userCondition;
				if(setting.isMixData){
					if(treeNode.condition!=undefined && treeNode.condition!=null){//是动态加载的节点
						if(treeNode.condition != ''){
							condition += ' AND ' + treeNode.condition;
						}
					}else if(setting.condition!=undefined && setting.condition!=null && setting.condition != ''){
						//是根节点
						condition += ' AND ' + setting.condition;
					}
				}else if(setting.condition!=undefined && setting.condition!=null && setting.condition != ''){
					condition += ' AND ' + setting.condition;
				}
		...
		var curNameCol = setting.nameCol;
		var curCondition = setting.condition;
		var curSortField = setting.sortField;
		var curOrder = setting.order;
		//当前树是混合数据
		if(setting.dwr && setting.isMixData){
			if(!setting.mixDataSelectHandler){
				alert("Error:使用混合数据需要mixDataSelectHandler方法");
				return;
			}
			setting.curDataName = setting.mixDataSelectHandler(treeNode.flagName,treeNode);//获取当前要使用的数据
			if(setting.curDataName && setting.curDataName != null){
				var curData = setting.mixData[setting.curDataName];
				if(curData!=undefined && curData!=null){
					setting.treeNodeKey = curData.treeNodeKey;
					curNameCol= curData.nameCol;
					setting.properties = curData.properties;
					setting.dwrFun = curData.dwrFun;
					setting.needLink = curData.needLink;
					curCondition = curData.condition;
					//将条件混合
					if(condition != undefined && condition != null && condition != ''){
						if(curCondition != undefined && curCondition != null && curCondition != ''){
							condition += ' AND ' + curCondition;
						}
					}else{
						if(curCondition != undefined && curCondition != null && curCondition != ''){
					...
					curSortField = curData.sortField;
					curOrder = curData.order;
				}
			}else{
				alert("Error:mixDataSelectHandler方法没有返回正确的值");
				return;
			}
		}
		//使用dwr
		if(setting.dwr && setting.dwrFun){
			setting.dwrFun(setting.properties, condition, 
					treeNode.sortField!=undefined ? treeNode.sortField :setting.sortField, 
					treeNode.order!=undefined ? treeNode.order : setting.order, 
					setting.needLink, function(msg) {
				var newNodes = [];
				try {
					if (!msg || msg.length == 0) {
						newNodes = [];
				...
				for (i = 0; i < newNodes.length; i++) {
					newNodes[i].flagName = setting.curDataName;
					newNodes[i].nameCol = curNameCol;
//					newNodes[i].condition = curCondition;
					newNodes[i].sortField = curSortField;
					newNodes[i].order = curOrder;
					//判断是否是叶子节点
					if(setting.isLeaf){
	...
	function selectNode(setting, treeNode) {
		...
		if (setting.editable && treeNode.editNameStatus) {
			...
			inputObj.attr("value", treeNode[treeNode.nameCol ? treeNode.nameCol : setting.nameCol]);
			tools.inputFocus(inputObj);
	...
	function zTreePlugin(){
		return {
			...
			getNodesByParam : function(key, value, parentNode) {
				if (!key) return null;
				//--------------这里的parentNode[this.setting.nodesCol]始终是null----------------------
//				return getTreeNodesByParam(this.setting, parentNode?parentNode[this.setting.nodesCol]:this.setting.root[this.setting.nodesCol], key, value);
				return getTreeNodesByParam(this.setting, parentNode?$(parentNode).attr(this.setting.nodesCol):this.setting.root[this.setting.nodesCol], key, value);
			},
			...
			cancelInput: function(newName) {
				...
				st.cancelPreEditNode(this.setting, newName?newName:treeNode[treeNode.nameCol ? treeNode.nameCol : this.setting.nameCol]);
