...
import com.cqut.util.stringUtil.StringUtil;

/**
 * 图片处理的工具类，包括裁减图片等功能，可以应用在上传图片后对图片进行切割等场合
 * 
 * @author huangkai
 */
public class ImageUtil {
	...
	/**
	 * 对图片进行裁减处理  .bmp和.jpg文件正常
	 * 
	 * @param srcImage		原始图片
	 * @param destImage 	裁减后的图片
	 * @param x     		开始裁减的 x 坐标
	 * @param y   		   	开始裁减的 y 坐标
	 * @param width  		裁减的宽度
	 * @param height 		裁减的高度
	 * @throws IOException
	 */
	public static void createPreviewImage(String srcImage, String destImage, int x, int y, int width, int height) throws IOException {
		// 在内存中生成待处理的图像
